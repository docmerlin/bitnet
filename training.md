...
## Full-Stack Benchmark Policy

Any benchmark described as **full**, **end-to-end**, or **production** must measure sustained training from raw bytes through the complete BLT + BitNet path: data ingestion, entropy patching, BLT local encoding and pooling, the global/main LLM, local decoding and loss, backward, gradient accumulation, clipping, and the optimizer update. It must use production feature settings, including MTP, quantization, activation checkpointing, and other enabled model branches. Preloaded tensors, synthetic token batches, isolated global-model steps, and optimizer-only runs are component diagnostics, not full-stack results.

Primary throughput = non-padding raw input **bytes/second**, counted before patching or tokenization. Also report seconds/update, peak unified memory, physical parameter count, bytes/update, patches/update, and patches/byte. Record power mode and thermal conditions; compare variants with interleaved runs after warmup and explicit MLX synchronization. Token/second and patch/second may remain secondary diagnostics but must not represent full-system training performance.
...
Full BLT + BitNet MLX benchmark, 2026-08-04: byte MTP belongs after BLT's causal local decoder, not on variable-width patch latents. Depth-4 sampled byte MTP rotates one head per accumulated microbatch and reuses the byte output head; the global adapter keeps patch-level `mtp_depth=0` deliberately. Fixing fixed-count entropy patching to return exactly the requested unpadded shape made the 980.28M-parameter path compilable. On M1 Max AC/high-power mode, raw-byte ingestion → entropy patching → BLT encoder → `8 + 48×4 + 8` BitNet global → BLT decoder/MTP loss → backward → four-way accumulation → clip → int8 CMUD sustained 225.35 bytes/s, 9.09s/update, 2,048 bytes/update, 512 patches/update, and 15.57 GiB peak. Previous eager variable-shape run was 27.85 bytes/s and 17.50 GiB: 8.09x throughput, 1.93 GiB less peak. Patcher weights were randomly initialized because no trained entropy checkpoint exists locally; this is structural throughput, not a quality result.
