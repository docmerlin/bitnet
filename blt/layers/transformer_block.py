...
class TernarySelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attention_mask: torch.Tensor | None = None) -> torch.Tensor:
        ...
        if self.causal:
            ...
            scores = scores.masked_fill(invalid.view(1, 1, seq_len, seq_len), mask_floor)
        ...
        if attention_mask is not None:
            if attention_mask.ndim == 2:
                ...
                scores = scores.masked_fill(~key_valid, mask_floor)
            elif attention_mask.ndim == 3:
                additive_mask = attention_mask[:, None, :seq_len, :seq_len].to(dtype=scores.dtype)
                scores = scores + additive_mask
                query_valid = additive_mask.amax(dim=-1, keepdim=True) >= 0
            elif attention_mask.ndim == 4:
                additive_mask = attention_mask[:, :, :seq_len, :seq_len].to(dtype=scores.dtype)
                scores = scores + additive_mask
                query_valid = additive_mask.amax(dim=-1, keepdim=True) >= 0
            else:
